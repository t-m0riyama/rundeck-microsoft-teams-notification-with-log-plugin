...
* 本ツールの実行環境に関する要件
  ...
  * Rundeck: 3.0以上（**5.0以上を主対象**。3.x でも後方互換を維持）
  * 通信要件など
    * TeamsのWebhook URLへのHTTPS（443）での接続性が必要
    * Rundeck APIへのHTTP(S)での接続性が必要
...
5. Rundeck API 接続（ログ取得用）  
ジョブ出力を通知に含める場合、プラグインは Rundeck API でログを取得する。接続先は **追加設定不要** で、以下の順に自動解決する（ジョブや project ごとの設定は不要）。

1. `framework.properties` の `framework.server.url` から API URL を導出（`{url}/api`）
   - Docker 公式イメージ: `/home/rundeck/etc/framework.properties`
   - RPM/DEB: `/etc/rundeck/framework.properties`
2. API バージョンを Rundeck API から自動検出
3. 上記が使えない場合のフォールバック: `https://localhost:4443/api` / バージョン `31`

Rundeck 5.x Docker では `framework.server.url = http://localhost:4440` が設定されているため、通常はそのまま動作する。ログ取得に失敗した場合、プラグインは **nolog テンプレート** にフォールバックし、Rundeck のサーバログにエラーを出力する。

## Rundeck 5.x への移行

Rundeck 3.x から 5.x へ移行する際は、以下を実施する。

### 1. プラグインの更新

`src/` 配下を `$RUNDECK_PLUGIN_DIR` にコピーし、Rundeck を再起動する（「インストール」手順と同様）。

### 2. ジョブ定義の `type` 変更（必須）

Rundeck 5.x では、ジョブ YAML / エクスポート定義の通知プラグイン `type` に **Groovy ファイル名（拡張子なし）** を指定する。UI の表示名（title）ではない。

| Rundeck 3.x（旧） | Rundeck 5.x（新） |
|-------------------|-------------------|
| `Microsoft Teams notification with log output` | `MicrosoftTeamsNotificationWithLog` |
...
既存ジョブは一括でエクスポートし、`type` を置換してから再インポートするか、UI から通知設定を開き直して保存する。

### 3. framework.properties の確認

`framework.server.url` が Rundeck の実際の URL と一致していることを確認する。公式 Docker イメージでは remco が自動設定するため、通常は変更不要。

### 4. デフォルトテンプレート

新規ジョブのデフォルトテンプレートは `SimpleMessage-AdaptiveCard`（Adaptive Card 形式）に変更されている。従来の MessageCard 形式を使う場合は `template_name: SimpleMessage` を明示する。
...
～省略～
{
    ...
    "themaColor": "${color}",
    "summary": "***[Rundeckジョブ結果通知 / ${notification_label}] #${execution.id}***",
    "title": "[Rundeckジョブ結果通知 / ${notification_label}] #${execution.id}",
    "sections": [
        {
